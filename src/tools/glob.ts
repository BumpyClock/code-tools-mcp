...
export async function globTool(input: GlobInput) {
  ...
  if (filtered.length === 0) {
    ...
    const ignoreNote = gitIgnoredCount > 0 ? ` (${gitIgnoredCount} files ignored by .gitignore)` : '';
    return { 
      content: [{ type: 'text' as const, text: `No files found matching "${input.pattern}" in ${baseDirRel}${ignoreNote}` }], 
      structuredContent: { 
        files: [], 
