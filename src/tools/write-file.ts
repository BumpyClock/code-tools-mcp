...
function mapFileSystemError(error: any): { message: string; code: string } {
  if (error?.code === 'EACCES') {
    return { message: 'Permission denied: cannot write to file', code: 'PERMISSION_DENIED' };
  } else if (error?.code === 'ENOSPC') {
    return { message: 'No space left on device', code: 'DISK_FULL' };
  } else if (error?.code === 'EISDIR') {
    return { message: 'Cannot write to directory: path is a directory', code: 'IS_DIRECTORY' };
  } else if (error?.code === 'ENOTDIR') {
    return { message: 'Parent path is not a directory', code: 'PARENT_NOT_DIRECTORY' };
  }
  return { message: error?.message || 'Write operation failed', code: 'WRITE_FAILED' };
}
...
export async function writeFileTool(input: WriteFileInput) {
  ...
  try {
    ...
    return { 
      content: [{ type: 'text' as const, text: `Cannot write outside workspace: ${file_path}` }], 
      structuredContent: { error: 'OUTSIDE_WORKSPACE', message: 'Path is outside the workspace root' } 
    };
  }
  ...
  try {
    ...
    return { 
      content: [{ type: 'text' as const, text: `Failed to write file: ${errorInfo.message}` }], 
