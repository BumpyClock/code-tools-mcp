...
export const grepShape = {
  ...
  exclude: z.union([z.string(), z.array(z.string())]).optional().describe('Optional glob to exclude, e.g. **/dist/**'),
  ...
  max_matches: z.number().optional().describe('Maximum number of matches to return (default 2000).'),
};
...
export async function grepTool(input: GrepInput, signal?: AbortSignal) {
  ...
  const excludePatterns = input.exclude 
    ? Array.isArray(input.exclude) ? input.exclude : [input.exclude]
    : undefined;
  
  const files = await fg(include, { cwd: baseDir, absolute: true, dot: true, ignore: excludePatterns });
  ...
  if (matches.length === 0) {
    ...
    return { 
      content: [{ type: 'text' as const, text: `No matches for "${input.pattern}" in ${where}${filter}.` }], 
      structuredContent: { 
        matches: [], 
  ...
  const matchesByFile = new Map<string, Array<{ lineNumber: number; line: string }>>();
  for (const match of matches) {
    if (!matchesByFile.has(match.filePath)) {
      matchesByFile.set(match.filePath, []);
    }
    matchesByFile.get(match.filePath)!.push({ lineNumber: match.lineNumber, line: match.line });
  }
